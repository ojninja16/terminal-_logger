...
const service = {
  ...
  logEntries: () => FIND_MANY_RESULT,
  logEntry: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("LogEntry", () => {
  ...
  test("GET /logEntries/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/logEntries"}/${nonExistingId}`)
  ...
  test("GET /logEntries/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/logEntries"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /logEntries existing resource", async () => {
